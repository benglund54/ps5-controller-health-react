...
function App() {
  ...
  const [homeSession, setHomeSession] = useState(() => createHomeSession(playerPersonas[0]?.ownerId));
  ...
  const activePersona = useMemo(
    () => playerPersonas[activePersonaIndex] ?? playerPersonas[0],
    [activePersonaIndex]
  );

  const maxPickerIndex = playerPersonas.length;
  ...
  const openSelectedPickerItem = useCallback((index) => {
    ...
    const persona = playerPersonas[personaIndex];
  ...
  const handleManualSelect = useCallback((query) => {
    ...
    const foundIndex = playerPersonas.findIndex((persona) =>
    ...
    setHomeSession(createHomeSession(playerPersonas[foundIndex]?.ownerId));
    setScreen("home");
